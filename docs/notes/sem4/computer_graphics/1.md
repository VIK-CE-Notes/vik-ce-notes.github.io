...
- Egységvektor: $\displaystyle|v^0| = 1, v^0 \cdot v^0 = 1, v^0 = \frac{v}{\sqrt{v \cdot v}}$
...
Ha $r$ helyére behelyettesítünk, akkor könnyen eldönthetjük, hogy egy pont rajta van-e (egyébként pont azért _implicit_ egyenlet, mert az $r$ egyenest nem fejezzük ki expliciten).
...
!!! info 1.12. Definíció (Egyenes (főkör) egyenlete)
...
??? info Kép
    ![középpontos vetítés](./img/1_central_proj.png)
...
??? info Kép
    ![sztereografikus vetítés](./img/1_stereo_proj.png)
...
??? info Kép
    ![mercator vetítés](./img/1_mercator_proj.png)
...
!!! info Tétel:
    Ha $p_1$ és $p_2$ pontra illeszkedik egy $l$ egyenes:
...
!!! info Tétel:
    Ha $l_1$ és $l_2$ pontra illeszkedik egy $p$ egyenes:
