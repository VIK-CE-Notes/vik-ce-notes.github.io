...
A hátránya az, hogy nagyon lassú. [Később](8.md/#összegzés) majd látunk egy példát konkrét számokkal, de általánosságban ha nem csak egy-egy képet szeretnénk generálni, hanem másodpercenként sokat, akkor legtöbbször ez a megközelítés alkalmatlan.
...
![pipeline](../img/chapter_5/5_pipeline.png){width=100%}
...
A vágás után már transzformálhatjuk a primitíveinket a képernyőkoordináta-rendszerbe, ami pedig a _nézet transzformáció_:
...
![line-strip](../img/chapter_5/5_line_strip.png){width=100%}
...
??? abstract Bizonyítás
    ...
    ![duális gráf](../img/chapter_5/5_vagasfa.png){width=100%}
...
??? example Részletesebben
    [Ebben](https://youtu.be/Mezx8ov5Akw&t=1095) a videóban részletesebben, ábrákkal el van magyarázva a második megközelítés.
...
<!-- NOTE: ötletem sincs, hogy ez a mondat mit jelent, de az előadásvideóban benne volt. nem fejti ki ott sem. -->
Megjelenik a view és projection mátrixok inverze is, ezekre a bemeneti csővezeték ágán lesz szükségünk.

### Összegzés

<!-- NOTE: ide jó lenne több magyarázat az MVP mátrixról -->
...
- `Draw()`: kirajzoljuk az objektumot. Ehhez a már megismert `Camera2D` osztály hívjuk segítségül. A vektorizáció után a modell, view és projection transzformációkat kell elvégezni.
    ...
    - végezetül pedig feltöltjük a mátrixunkat a GPU-ra, mint uniform változót, és meghívjuk a `Geometry` osztály kirajzoló függvényét, ami már az OpenGL függvényeit hívja meg.
...
![vágás](../img/chapter_5/5_vpvagas.png){width=100%}
...
??? example Poligon vágás
    A diákon és az [előadás-videóban](https://youtu.be/js8Q716kMUg&t=1347) részletezve van a poligon vágó algoritmus. Eme jegyzet írói szerint ez inkább csak érdekesség, a szorgos hallgatóknak ajánljuk abszolválásra.
...
Ez továbbra is inkrementális, de bit shift-el végzi el a kerekítést, aminek köszönhetően nagyon gyors. Ha ezt egy [DDA](https://en.wikipedia.org/wiki/Digital_differential_analyzer_(graphics_algorithm)) szakaszkirajzoló hardver végzi el, akkor még tovább növelhető a sebesség (3 regiszter, meg 1 összegző).
...
A fő felismerésünk az, hogy ha egy tulajdonság igaz egy pixelre, akkor nagy eséllyel a szomszédjaira is, hacsak valamilyen határt át nem lépünk. Csoportosítsuk a pixeleinket _pásztákba_, azaz hosszú vízszintes sorokba. A pászta elején sok pixel nem lesz benne a háromszögben, aztán lesz egy határpont, ahol átbillenünk, és hirtelen minden pixel a háromszögön belül van, aztán egy második határpont, ami után már egyik pixel sem lesz a háromszögön belül.
...
!!! question 4\. A szakasz két végpontjának koordinátái normalizált eszközkoordináta-rendszerben: $(-3.7,0.2,-0.5), (4.7,0.2,-0.5)$
...
!!! question 5\. Hány diagonálja van az alábbi 5-szögnek?
    ![kviz](../img/chapter_5/5_kviz_csillag.png){width=100%}
...
!!! question 10\. Hány füle van az alábbi 5-szögnek?
    ![quiz](../img/chapter_5/5_kviz_csillag.png){width=100%}
