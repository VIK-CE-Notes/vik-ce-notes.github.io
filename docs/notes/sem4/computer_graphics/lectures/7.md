...
Egy ilyen illúzió keltését úgy érhetjük el, hogy azok a fénysugarak, amik a képernyőről a szemünkbe érkeznek, a lehető legjobban hasonlítsanak azokra, amik a való világból bombázzák a szemünket. A szemünk viszont csak nagyon kevés információt képes kinyerni ezekből az elektromágneses hullámokból, tehát nekünk elég csak a fény _spektrumát_ reprodukálni.

??? example További érdekességek
    Bár a jegyzet írói szerint a tananyagnak szerves részét nem képezi, akit jobban érdekel ennek a tudományágnak az alapjai, milyen kísérletek voltak, illetve, hogy miért RGB képszintézist alkalmazunk, annak tudjuk ajánlani [ezt](https://www.youtube.com/watch?v=aYODY4kc0W4) a videót, melyben ezekről szó esik.
...
??? info Radiancia ábra
    ![radiancia](../img/chapter_7/7_radiancia.png){width=100%}
...
Ha két felület radianciája megegyezik, akkor _a két felület távolságától, és orientációjától függetlenül_ azokat a megfigyelő ugyan olyan színűnek látja, más szóval _a sugársűrűség egy sugár mentén állandó_.

??? abstract Bizonyítás
    Proof by [higher authority](https://youtu.be/XdXiAe0G6Gg&t=341). <span style="display: flex; justify-content: flex-end;"> $\blacksquare$ </span>
...
![fresnel](../img/chapter_7/7_fresnel.png){width=100%}
...
$$
n = \frac{\sin(\theta^{\text{in}})}{\sin(\theta)} \implies \theta = \arcsin\left(\frac{\sin(\theta^{\text{in}})}{n}\right)
$$
...
![reflection](../img/chapter_7/7_reflect.png){width=100%}
...
![refraction](../img/chapter_7/7_refract.png){width=100%}
...
$$
\boxed{\frac{v}{n} + \bm{N} \left( \cos(\alpha/n) - \sqrt{1 - \frac{1 - \cos^2{\alpha}}{n^2}} \right)}
$$

Ha a gyök alatt negatív van, az a gyakorlatban annak feleltethető meg, hogy nincsen törés, azaz minden foton visszaverődik (_teljes belső visszaverődés_).
...
![rücsök](../img/chapter_7/7_rucsi.png){width=100%}
...
Egy másik előnye a geometriai és anyagi tényezők szétbontásának, hogy a megvilágítás erőssége, azaz az _irradiancia_ csak a geometriai tényezővel arányos.
...
![spekuláris](../img/chapter_7/7_phong_split.png){width=100%}
...
&= L^{\text{in}} \left( k_d + k_s \cdot \frac{(\cos^{+}\rho)^{\text{shine}}}{\cos^{+}(\theta^{\text{in}})} \right) \cos^{+}(\theta^{\text{in}})
\end{align*}
}
$$
...
$$
\left( k_d + k_s \cdot \frac{(\cos^{+}\rho)^{\text{shine}}}{\cos^{+}(\theta^{\text{in}})} \right)
$$
...
$$
L^{\text{in}} \left( k_d + k_s \cdot \frac{(\cos^{+}\rho)^{\text{shine}}}{(\bm{L} + \bm{V})^2} \right) \cos^{+}(\theta^{\text{in}})
$$
...
![példa](../img/chapter_7/7_phong_example.png){width=100%}
...
??? tip Megoldás
    ...
    $$
    \theta^\text{in} = \arcsin\left(\frac{1}{10}\right) = 5.74\degree
    $$
