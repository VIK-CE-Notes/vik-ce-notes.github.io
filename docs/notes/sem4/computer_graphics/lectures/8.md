...
![raytrace](../img/chapter_8/8_raytrace.png){width=100%}
...
- a $\ast$ a spektrumok közötti szorzás (azaz `vec3` szorzás, tényezőnként külön-külön)
...
![closest](../img/chapter_8/8_closest.png){width=100%}
...
![sphere](../img/chapter_8/8_sphere_intersect.png){width=100%}
...
![háromszög metszés](../img/chapter_8/8_triangle.png){width=100%}
...
&= \text{lookat} + \left(\frac{2(X+0.5)}{\text{XM}-1}\right) \cdot \text{right} + \left(\frac{2(Y+0.5)}{\text{YM}-1}\right) \cdot \text{up}
\end{align*}
$$
...
![trace](../img/chapter_8/8_trace.png){width=100%}
...
??? example Miért is kell az $N_\varepsilon$?
    ...
    ![no_epsilon](../img/chapter_8/8_no_epsilon.png)
...
![recursive](../img/chapter_8/8_recurse.png){width=100%}
...
![class_diagram](../img/chapter_8/8_class_diagram.png){width=100%}
...
???+ example Térpartícionáló adatstruktúrák, globális illumináció
    Ez a jegyzet ezekkel a témákkal bővebben nem foglalkozik, viszont a hivatalos előadás fóliákon ezek a témák is megtalálhatóak. A jegyzet írói nem hinnék, hogy valaha is házi, vagy vizsga anyag lenne ez a két témakör, de a teljesség kedvéért itt is (röviden) meg vannak említve.

    ## Térpartícionáló adatstruktúrák

    !!! quote "Idézet"
        $\text{[...] the difference between a bad programmer and a good one is whether he [sic, read they] con-}\\
        \text{siders his code or his data structures more important.}$
        $\hspace{24em} \text{- Linus Torvalds}$
    ...
    $$
    L(r, \omega) = D(r, \omega) + \int_{\Omega} L(y, \omega') \cdot R(\omega, \omega') \, \mathrm{d}\omega'
    $$
    ...
    $$
    \begin{alignat*}{2}
    &L(r, \omega)    &&= D(r, \omega) + \int_{\Omega} L(y, \omega') \cdot R(\omega, \omega') \, \mathrm{d}\omega' \quad \text{ahol:} \\[3ex]
    &L(r, \omega')   &&= D(r, \omega') + \int_{\Omega'} L(y, \omega'') \cdot R(\omega', \omega'') \, \mathrm{d}\omega'' \quad \text{ahol:} \\[3ex]
    &L(r, \omega'')  &&= D(r, \omega'') + \int_{\Omega''} L(y, \omega''') \cdot R(\omega'', \omega''') \, \mathrm{d}\omega''' \quad \text{ahol:} \\
    &&\vdots
    \end{alignat*}
    $$
    ...
    $$
    \int_0^1 f(z) \, \mathrm{d}z \approx \frac{1}{M} \sum_{i = 1}^{M} f(z_i)
    $$
    ...
    $$
    \int f(z) \, \mathrm{d}z 
    = \int \frac{f(z)}{p(z)} \cdot p(z) \, \mathrm{d}z 
...
!!! question 7\. Az alábbi osztály egy origó középpontú és `axes.x`, `axes.y`, `axes.z` fél főtengelyhosszú ellipszoidot valósít meg, amelyre a sugár metszéspontszámítást implementáltuk. A `vec3`-ra az osztást GLSL stílusban, azaz koordinátánként függetlenül valósítottuk meg. Kérjük a hibás sorok sorszámát!
    ...
    public:
        Hit intersect(const Ray& ray) {
            ...
            float a = dot(ray.dir/axes, ray.dir/axes);
            float b = dot(ray.start/axes, ray.dir/axes) * 2;
            float c = dot(ray.start/axes, ray.start/axes) ;
...
??? tip Megoldás
    ...
    ```
    A 16. sor a hibás, a felületi normált így **csak** a gömbnél számolhatunk.
