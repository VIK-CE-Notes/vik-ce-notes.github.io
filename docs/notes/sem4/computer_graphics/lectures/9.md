...
$$
(\bm{T}^{-1}_{\text{Model}})^{T} \cdot [\bm{N}, 0]^{T} = [\bm{N}_{\text{world}}, d]^{T}
$$
...
???+ question Miért egészíthetjük ki egy $0$-val a normált?
    Röviden azért, mert mivel $(\bm{T}_{\text{Model}})^{T}$ egy affin transzformáció, ezért az inverze is az<sup>[[Citation needed.]](https://cdn.discordapp.com/emojis/1384476146636161065.webp?size=240)</sup>, tehát nem befolyásolja a normálvektortunk értékét a negyedik koordináta.
